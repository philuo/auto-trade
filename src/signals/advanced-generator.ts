...
import { AdvancedIndicatorCalculator } from '../indicators/advanced-indicators.js';
import { SignalType, SignalDirection } from '../market/types.js';
import type { KLineInterval, TechnicalSignal, CandleData } from '../market/types.js';
import type { AdvancedIndicators } from '../indicators/advanced-indicators.js';
...
export class AdvancedSignalGenerator {
  ...
  generateSignals(
    ...
    try {
      ...
      // 6. 布林带信号（带价格确认）
      signals.push(...this.generateBollingerSignals(coin, indicators, klines, prevState, currState, marketState, timeframe, timestamp));
  ...
  private generateBollingerSignals(
    ...
    if (price > bollinger.upper) {
      // 检查是否有成交量配合
      const volumeOK = !this.config.enableVolumeConfirmation; // 简化处理
    ...
    if (price < bollinger.lower) {
      const volumeOK = !this.config.enableVolumeConfirmation;
  ...
  private generateSignalId(type: SignalType, coin: string, timeframe: KLineInterval): string {
    return `${type}_${coin}_${timeframe}_${Date.now()}`;
  }
}
